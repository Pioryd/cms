...
{
  cms::string command;
  command = text_box_get_text(text_box_script);
  if (command == "exit")
  {
    cmsext::app::close();
    return;
...
void create_ui()
{
  ...
  cms::string label = label_create(root);
  ...
  cms::string button_run_as_python = button_create(root);
  ...
  cms::string button_run_as_command = button_create(root);
